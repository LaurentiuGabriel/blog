---
permalink: /
layout: layouts/page.njk
eleventyExcludeFromCollections: true
title: Hey, I'm Laurentiu
seoTitle: Laurentiu Raducu — Software Engineer, Founder & AI Builder
tagline: Software engineer, founder of Bitheap, AI builder. Based in Zürich, Switzerland.
description: Full-stack developer with 12+ years across software development, security and QA automation. Writing about AI, systematic trading and building things that ship.
---

I'm a full-stack developer with **15+ years** across software development, security
and QA automation. I spent the better part of a decade
building and breaking systems for financial institutions, and these days most of my
energy goes into AI products, systematic trading research and options analytics.

Since 2022 I've been running **[Bitheap](https://bitheap.ch)**, where I build and
ship the ideas for small and medium organizations.

## What I'm working on

- **[GPT Trading Agent](https://aivestor.tech)** — an LLM-driven agent that researches and executes trades.
- **[GreekFlow](https://github.com/LaurentiuGabriel/greek-alpaca)** — options analytics: Greeks, flow and volatility surfaces.
- **[Select Supply](https://select.supply)** — clothes, cars, games and other objects that I love.
- **[Dave](https://dave-bot.com)** — a one-stop shop for everything AI-related.
- **[QuantiQ](https://quantiq.live)** — real-time market, historical and financial data.

## The toolbox

**Languages** — Python, JavaScript, TypeScript, Java, Kotlin, C++, C#, Rust, Go, SQL

**Frameworks** — React, Vue, Svelte, Angular, Spring, Django, Three.js

**AI & quant** — LLM agents, OpenAI API, RAG, Alpaca API, Black-Scholes, backtesting

**DevOps** — Docker, Kubernetes, Terraform, Azure, Jenkins

## Teaching

Somewhere along the way I ended up teaching more than 100,000 students — 75k+ on
Udemy, plus courses on Pluralsight. I've written four books on Amazon, I write on
Medium and the Pluralsight blog, and I put videos on YouTube.

---

This is where I keep the longer-form notes. Start with the
[articles](/articles/), or [say hello](/contact/).
